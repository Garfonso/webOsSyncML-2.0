...
kill:
	plink -P $(TARGETPORT) root@$(TARGETHOST) -pw "" killall -9 webOsSyncML
	
run:
	plink -P $(TARGETPORT) root@$(TARGETHOST) -pw "" "/media/internal/test/webOsSyncML"
...
install: all
	pscp -scp -P $(TARGETPORT) -pw "" webOsSyncML root@$(TARGETHOST):/media/cryptofs/apps/usr/palm/applications/info.mobo.webossyncml
